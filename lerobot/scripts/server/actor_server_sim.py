...
def debug_tensor(obj, name="obj"):
    if isinstance(obj, torch.Tensor):
        if obj.isnan().any():
            print(f"[DEBUG] NaN found in {name}")
        if not obj.isfinite().all():
            print(f"[DEBUG] Inf found in {name}")
    elif isinstance(obj, dict):
        for k, v in obj.items():
            debug_tensor(v, name=f"{name}[{k}]")
...
def act_with_policy(cfg, reward_classifier, shutdown_event, parameters_queue, transitions_queue, interactions_queue):
    log_dir = os.path.join(cfg.output_dir, "logs")
    os.makedirs(log_dir, exist_ok=True)
    ...
    with mujoco.viewer.launch_passive(online_env.model, online_env.data, show_left_ui=False, show_right_ui=False,) as viewer:
        for interaction_step in range(cfg.policy.online_steps):
            ...
            if interaction_step >= cfg.policy.online_step_before_learning:
                with TimerManager(elapsed_time_list=list_policy_time, label="Policy inference time", log=False):
                ...
                action = torch.from_numpy(action[0]).to(device).unsqueeze(dim=0)
            ...
            debug_tensor(action, name="action")
            debug_tensor(obs, name="obs")
            debug_tensor(next_obs, name="next_obs")
            ...
            if done or truncated:
                interactions_queue.put(
                    python_object_to_bytes({
                        ...
                        "Intervention rate": episode_intervention_steps / episode_total_steps if episode_total_steps > 0 else 0.0,
                    })
                )
                transitions_queue.put(transitions_to_bytes(list_transition_to_send_to_learner))
                list_transition_to_send_to_learner.clear()
...
def establish_learner_connection(stub, shutdown_event, attempts=30):
    for _ in range(attempts):
        if shutdown_event.is_set():
            return False
        try:
    ...
    while not shutdown_event.is_set():
        try:
            ...
            if not isinstance(message, bytes):
                print("[ACTOR] ERROR: Got non-bytes in transitions_queue:", type(message))
                continue

            yield from send_bytes_in_chunks(
                message,
                hilserl_pb2.Transition,
                log_prefix="[ACTOR] Send transitions"
            )

        except Empty:
            continue  # queue is temporarily empty
        except (EOFError, OSError, ValueError) as e:
            print(f"[ACTOR] Known queue error: {e}")
            break
        except Exception as e:
            print(f"[ACTOR] Unexpected error in transitions_stream: {e}")
            break

    print("[ACTOR] Transition stream exiting.")
    return hilserl_pb2.Empty()
    ...
    for transition in transitions:
        ...
        for key, value in tr["state"].items():
            if torch.isnan(value).any():
                print("Found NaN values in transition")
...
def log_policy_frequency_issue(policy_fps: float, cfg: TrainPipelineConfig, interaction_step: int):
    if policy_fps < cfg.env.fps:
        print("Policy FPS {policy_fps:.1f} below required {cfg.env.fps} at step {interaction_step}")
        # logging.warning(
