...
class BatchTransitionNSteps(TypedDict):
    """Batch transition for N-step returns (used by ACFQL and similar algorithms)."""
...
class ReplayBufferNSteps(ReplayBuffer):
    """
    Extended replay buffer with N-step returns support for algorithms like ACFQL.

    This buffer inherits from ReplayBuffer and adds support for sampling
    n-step transitions with cumulative rewards, masks, and terminals.
    """
    ...
    def _initialize_storage(
        ...
        # Pre-allocate tensors for storage - use zeros instead of empty for NSteps
        self.states = {
    ...
    def sample_nstep_full(
        ...
        """Sample a random batch of transitions with n-step returns.
        ...
        Returns:
            BatchTransitionNSteps: Batched transitions with n-step information
        """
        if not self.initialized:
            raise RuntimeError("Cannot sample from an empty buffer. Add transitions first.")
        if n_steps <= 0:
    ...
    def sample_nstep_full_for_indices(
        ...
        """Sample n-step transitions for given indices.

        Args:
            idx (torch.Tensor): Indices to sample from
    ...
    def get_iterator_nstep(
        ...
        """
        Creates an infinite iterator that yields batches of n-step transitions.
        Will automatically restart when internal iterator is exhausted.
        ...
        while True:  # Create an infinite loop
            if async_prefetch:
                ...
                iterator = self._get_naive_iterator_nstep(batch_size, n_steps, gamma, queue_size)
    ...
    def _get_naive_iterator_nstep(
        ...
        """
        Creates a simple non-threaded iterator that yields n-step batches.
        ...
        Yields:
            BatchTransitionNSteps: Batch transitions with n-step information
        """
        import collections
    ...
    def _get_async_iterator_nstep(
        ...
        """
        Create an iterator that continuously yields prefetched n-step batches in a
        background thread. The design is intentionally simple and avoids busy
        waiting / complex state management.
    ...
    def from_lerobot_dataset(
        ...
        """
        Convert a LeRobotDataset into a ReplayBufferNSteps.
        ...
        Returns:
            ReplayBufferNSteps: The replay buffer with dataset transitions.
        """
        if capacity is None:
            capacity = len(lerobot_dataset)
        ...
        list_transition = cls._lerobotdataset_to_transitions_generator(
    ...
    def _lerobotdataset_to_transitions_generator(
        ...
        This is a memory-efficient generator version that yields transitions one at a time.
        ...
        Yields:
            Transition: Individual transition dictionaries.
        """
        if state_keys is None:
            raise ValueError("State keys must be provided when converting LeRobotDataset to Transitions.")
        ...
        for i in tqdm(range(num_frames)):
            ...
            # Handle truncation separately for n-step returns
            truncated = False
            if not done:
                # This is important if the dataset has truncations
                if i == num_frames - 1:
                    truncated = True
                elif i < num_frames - 1:
                    next_sample = dataset[i + 1]
...
def concatenate_batch_transitions_nstep(
    ...
    # Concatenate n-step specific fields
