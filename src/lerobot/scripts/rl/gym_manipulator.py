...
def record_dataset_with_embeddings(
    ...
    while episode_index < cfg.num_episodes:
        ...
        while time.perf_counter() - start_episode_t < cfg.wrapper.control_time_s:
            ...
            frame = {
                **temp_transition["state"],
                "action": temp_transition["action"],
                "next.reward": np.array([temp_transition["reward"]], dtype=np.float32),
                "next.done": np.array([temp_transition["done"]], dtype=bool),
                "complementary_info.discrete_penalty": torch.tensor(
                    [temp_transition["complementary_info"].get("discrete_penalty", 0.0)], dtype=torch.float32
                ),
            }
